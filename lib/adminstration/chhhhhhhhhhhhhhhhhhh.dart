...
class chtest extends StatefulWidget {
  final String name;
  final String userId;
  ...
  @override
  State<chtest> createState() => _chtestState();
}

class _chtestState extends State<chtest> {
  ...
  enregistrerMsgEnseignant(String msg, int chatId) async {
    Map<String, String> headers = {
      'Authorization': 'Bearer 35|zXhcGHqJLchDEUzlBUvqB70NnzpMbc2Hsk7JUg8X',
      'Content-Type': 'application/json',
      'Accept': 'application/json',
    };
    ...
    final response = await http.post(
      Uri.parse('http://192.168.1.37/ISIMM_eCampus/public/api/enseignant/contacter_admin'),
      headers: headers,
      body: json.encode({
        'chat_id': 7,
        'text':msg,
  ...
  Future<Map<String, dynamic>> getMsgEtudient(int selectedindex)async{
    ...
    http.Response response = await http.get(Uri.parse('http://192.168.1.37/ISIMM_eCampus/public/api/chat_messages_enseignant/7'),
        headers: headers);
